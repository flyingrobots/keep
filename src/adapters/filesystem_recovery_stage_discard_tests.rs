...
use fixture::{DiscardFixture, evidence, request};
...
fn exact_stage_discard_and_absent_retry_are_durable_for_every_parent() -> Result<(), Box<dyn Error>>
{
    ...
    let cases: [(RecoveryStage, &[u8]); 3] = [
        (RecoveryStage::Segment, b"s"),
        (RecoveryStage::Catalog, b"c"),
        (RecoveryStage::NextHead, b"h"),
    ];
...
fn changed_stage_evidence_is_preserved_and_refused_before_unlink() -> Result<(), Box<dyn Error>> {
    ...
    let expected = request(RecoveryStage::Segment, b"old")?;
    fs::write(fixture.stage_path(RecoveryStage::Segment), b"new")?;
    let observed = evidence(RecoveryStage::Segment, b"new")?;
    let mut discarder = fixture.discarder()?;
...
fn symbolic_stage_is_never_followed_or_removed() -> Result<(), Box<dyn Error>> {
    ...
    let request = request(RecoveryStage::Segment, b"outside")?;
