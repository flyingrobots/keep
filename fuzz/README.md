...
versions and resource limits. `cargo xtask fuzz` parses that file without shell
...
Before use and retention, `cargo xtask fuzz check-corpus` refuses links,
nonregular files, unknown target directories, oversized inputs, excess files,
and excess total bytes under the limits in `campaign.env`. Refused restored
state is discarded; refused post-campaign state is neither cached nor
uploaded.
