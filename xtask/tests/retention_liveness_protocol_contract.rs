...
fn generation_transitions_and_gc_snapshots_fail_closed() {
    ...
    for required in [
        ...
        "Before replacing `HEAD`, version-2 catalog publication",
        "every current retained closure against the candidate catalog",
        "Candidate publication refuses before mutation",
        ...
        "Version 2 does not delete or reuse namespace identity",
        ...
        "`ReaderFence`",
        "shared lock on persistent `reader.lock` before opening `HEAD`",
        "entire snapshot lifetime",
        "exclusive `reader.lock`",
        "new readers wait and existing readers drain",
        "root generation and canonical digest",
        "catalog generation and canonical digest",
        "complete coordinates before mutation",
        "catalog generation",
...
fn alternatives_and_evidence_boundaries_are_explicit() {
    ...
    for required in [
        ...
        "ordinary reconstruction anchor cannot retain a particular representation",
        "future representation-aware policy extension",
        "distinct `LayoutId` may use its own anchor",
        "physical claim",
