...
- Stage publishers and fixed-record readers share one exact-record module for
  ...
  the retention stage, the migration fixed-record stage, and the retention
  current-state reader consume it, and a contract test keeps ported modules
  from reimplementing those primitives.
