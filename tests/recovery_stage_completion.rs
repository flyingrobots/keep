//! Exact complete-stage recovery laws.

#[path = "recovery_stage_completion/execution_laws.rs"]
mod execution_laws;
#[path = "recovery_stage_completion/planning_laws.rs"]
mod planning_laws;
#[path = "recovery_stage_completion/retry_laws.rs"]
mod retry_laws;
#[path = "recovery_stage_completion/storage_double.rs"]
...
use keep::{
    LayoutEntryLimit, RecoveryStage, RecoveryStageAssessment, RecoveryStageCompletionRequest,
...
fn completion_request(
    ...
    Ok(plan_recovery_stage_completion(&assessment(
        stage, encoded,
    )?)?)
}
