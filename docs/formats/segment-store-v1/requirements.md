...
| `KEEP-CATALOG-004` | Every admitted catalog location equals a verified top-level record span in the exact named segment; every supplied segment is referenced and each referenced segment is scanned once | Bounded grouped lookup plan and golden artifacts | `tests/catalog_locations.rs` | Implemented in #16 |
...
| `KEEP-CATALOG-008` | Segment, catalog, and head publication follows the documented file and directory synchronization order | Fault-recording port and filesystem fixtures | `tests/catalog_publication.rs`, `tests/catalog_filesystem_publication.rs` | Implemented in #16 |
