...
| `KEEP-RETENTION-004` | Retain and release compare expected and observed generations and publish exact successors only | storage-independent planning and generation-before-closure preflight in `tests/retention_transition.rs` and `tests/retention_preflight.rs`; publication evidence remains | In progress in #19 |
