...
| `KEEP-RETENTION-005` | Closure derivation is deterministic, bounded, cycle-safe, fail-closed, and verifies complete blob reconstruction | exact accounting plus one-anchor and repeated-chunk laws in `tests/retention_closure.rs`; property, corruption, and adversarial catalog tests remain | In progress in #19 |
