...
| `KEEP-RETENTION-002` | Root, manifest, and head codecs implement the exact canonical grammars and fixed bounds | independent golden corpus plus root encoder and decoder evidence in `tests/retention_root_encoding.rs` and `tests/retention_root_decoding.rs`; manifest/head codecs remain | In progress in #19 |
...
| `KEEP-RETENTION-009` | Exact already-committed retry is idempotent only while its successor remains current | retry and stale-successor tests | Planned in #19 |
